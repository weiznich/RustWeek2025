...
#[expect(
    clippy::unused_async,
    reason = "Resolving the todo will make this function async"
)]
...
#[expect(
    clippy::unused_async,
    reason = "Implementing the todo will make the function async"
)]
...
async fn render_add_participant(
    ...
    let competition_id = match redirect_parts.as_slice() {
        ["competitions", _, _] => id,
        ["races", _, _] => {
            participant.participant.race_id = Some(id);
            todo!("Load the competition id based on the race_id")
        }
        ["starts", _, _] => {
